...
from saptase.core.models import Molecule, SaptTask, SaptResult, TaskStatus
...
def _insert_dummy_data(db: LogDb):
    ...
    # Task 1: Original
    task1_orig = SaptTask(id="task1_orig", monomer_a=MOL_A, monomer_b=MOL_B_1, basis_set="sto-3g", method="sapt0")
    result1_orig = SaptResult(task_id="task1_orig", success=True, energies={"total": -1.0}, basis_set="sto-3g", method="sapt0", monomer_a_xyz=MOL_A_XYZ, monomer_b_xyz=MOL_B_XYZ_1, timestamp_utc="2023-01-01T00:00:00Z", attempt_number=1, elapsed_time=0.1)
    ...
    task2_dup = SaptTask(id="task2_dup", monomer_a=MOL_A, monomer_b=MOL_B_1, basis_set="sto-3g", method="sapt0")
    result2_dup = SaptResult(task_id="task2_dup", success=True, energies={"total": -1.1}, basis_set="sto-3g", method="sapt0", monomer_a_xyz=MOL_A_XYZ, monomer_b_xyz=MOL_B_XYZ_1, timestamp_utc="2023-01-01T01:00:00Z", attempt_number=1, elapsed_time=0.1)
    ...
    task3_newest_dup = SaptTask(id="task3_newest_dup", monomer_a=MOL_A, monomer_b=MOL_B_1, basis_set="sto-3g", method="sapt0")
    result3_newest_dup = SaptResult(task_id="task3_newest_dup", success=True, energies={"total": -1.2}, basis_set="sto-3g", method="sapt0", monomer_a_xyz=MOL_A_XYZ, monomer_b_xyz=MOL_B_XYZ_1, timestamp_utc="2023-01-01T02:00:00Z", attempt_number=1, elapsed_time=0.1)
    db.log_task_attempt(run_id="run1", result=result3_newest_dup) # log_id should be 3
    
    # Task 4: Unique task (different basis set)
    task4_unique_basis = SaptTask(id="task4_unique_basis", monomer_a=MOL_A, monomer_b=MOL_B_1, basis_set="6-31g", method="sapt0")
    result4_unique_basis = SaptResult(task_id="task4_unique_basis", success=True, energies={"total": -2.0}, basis_set="6-31g", method="sapt0", monomer_a_xyz=MOL_A_XYZ, monomer_b_xyz=MOL_B_XYZ_1, timestamp_utc="2023-01-01T03:00:00Z", attempt_number=1, elapsed_time=0.1)
    db.log_task_attempt(run_id="run1", result=result4_unique_basis) # log_id should be 4

    # Task 5: Unique task (different monomer B)
    task5_unique_monB = SaptTask(id="task5_unique_monB", monomer_a=MOL_A, monomer_b=MOL_B_2, basis_set="sto-3g", method="sapt0")
    result5_unique_monB = SaptResult(task_id="task5_unique_monB", success=True, energies={"total": -3.0}, basis_set="sto-3g", method="sapt0", monomer_a_xyz=MOL_A_XYZ, monomer_b_xyz=MOL_B_XYZ_2, timestamp_utc="2023-01-01T04:00:00Z", attempt_number=1, elapsed_time=0.1)
...
def test_deduplicate_keep_oldest(db_with_duplicates: LogDb, tmp_path: Path):
    ...
    removed_count = db_with_duplicates.deduplicate(overwrite=False)
...
def test_deduplicate_keep_newest(tmp_path: Path):
    ...
    removed_count = db.deduplicate(overwrite=True)
...
def test_deduplicate_no_duplicates(tmp_path: Path):
    ...
    task1 = SaptTask(id="task1", monomer_a=MOL_A, monomer_b=MOL_B_1, basis_set="sto-3g", method="sapt0")
    result1 = SaptResult(task_id="task1", success=True, energies={"total": -1.0}, basis_set="sto-3g", method="sapt0", monomer_a_xyz=MOL_A_XYZ, monomer_b_xyz=MOL_B_XYZ_1, attempt_number=1, elapsed_time=0.1)
    db.log_task_attempt(run_id="run1", result=result1)

    task2 = SaptTask(id="task2", monomer_a=MOL_A, monomer_b=MOL_B_1, basis_set="6-31g", method="sapt0") # Different basis
    result2 = SaptResult(task_id="task2", success=True, energies={"total": -2.0}, basis_set="6-31g", method="sapt0", monomer_a_xyz=MOL_A_XYZ, monomer_b_xyz=MOL_B_XYZ_1, attempt_number=1, elapsed_time=0.1)
    db.log_task_attempt(run_id="run1", result=result2)

    assert len(get_all_task_ids(db_path)) == 2
    
    removed_count_false = db.deduplicate(overwrite=False)
    assert removed_count_false == 0
    assert len(get_all_task_ids(db_path)) == 2 # Count should not change
    
    removed_count_true = db.deduplicate(overwrite=True)
    assert removed_count_true == 0
    assert len(get_all_task_ids(db_path)) == 2 # Count should not change 