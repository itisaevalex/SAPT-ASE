...
class MockFailureBackend(SaptBackend):
    ...
    def __init__(self, fail_on_basis=None, fail_scf_original_ids=None, fail_memory_original_ids=None, success_on_basis=None, success_on_keywords=None, fail_always_original_ids=None):
        self.fail_on_basis = fail_on_basis # Basis to fail on first attempt
        self.fail_scf_original_ids = fail_scf_original_ids or [] # Original IDs to fail SCF on first attempt
        self.fail_memory_original_ids = fail_memory_original_ids or [] # Original IDs to fail Memory on first attempt
        self.success_on_basis = success_on_basis # Basis to succeed on (usually for basis recovery)
        self.success_on_keywords = success_on_keywords or {} # Keywords needed for success (usually for SCF/Mem recovery)
        self.fail_always_original_ids = fail_always_original_ids or [] # Original IDs to always fail, simulating unrecoverable errors
    ...
    def calculate(self, task: SaptTask) -> SaptResult:
        ...
        logger.debug(f"MockBackend: Task {task.id} (Original: {original_task_id}), Attempt {attempt+1}, Basis {task.basis_set}, Keywords {task.additional_keywords}")
        ...
        if original_task_id in self.fail_always_original_ids:
             ...
             raise ScfFailed(f"Simulating persistent failure for {original_task_id}, attempt {attempt+1}") # Use ScfFailed for simplicity
        ...
        if attempt == 0:
            ...
            if self.fail_on_basis and task.basis_set == self.fail_on_basis:
                 logger.debug(f"MockBackend: Simulating BasisIncompatible for {task.id} on basis {task.basis_set}")
        ...
        if self.success_on_keywords and all(kw in task.additional_keywords and task.additional_keywords[kw] == self.success_on_keywords[kw] for kw in self.success_on_keywords):
             logger.debug(f"MockBackend: Simulating Success for {task.id} based on keywords {task.additional_keywords}")
             result = SaptResult(
                 task_id=task.id,
                 success=True,
        ...
        if attempt > 0:
             logger.debug(f"MockBackend: Simulating failure on retry attempt {attempt+1} for task {task.id} as success conditions not met.")
...
def test_orchestrator_recover_scf_failed(sample_task, log_db):
    ...
    backend = MockFailureBackend(
        fail_scf_original_ids=[sample_task.id],
        success_on_keywords=expected_recovery_keywords
    )
    ...
    assert final_result.attempt_number == 1, f"Final successful attempt should be attempt 1, got {final_result.attempt_number}."
...
def test_orchestrator_exhaust_ladder(sample_task, log_db):
    ...
    backend = MockFailureBackend(
        fail_always_original_ids=[sample_task.id]
    )
    ...
    assert final_result.error_code == 'ScfFailed', f"Expected final error code 'ScfFailed', got '{final_result.error_code}'"
    ...
    expected_logs = 2
    assert len(logs) == expected_logs, f"Expected {expected_logs} log entries for the task and its retries, found {len(logs)}."
