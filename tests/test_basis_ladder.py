from saptase.core.basis import (BASIS_LADDER, get_basis_rung, get_df_basis,
                               get_next_basis, get_previous_basis)
...
def test_basis_ladder_navigation():
    ...
    assert get_next_basis("jun-cc-pvqz") == "aug-cc-pvqz", \
        "Next after jun-cc-pvqz should be aug-cc-pvqz"
    assert get_next_basis("aug-cc-pvqz") is None, \
        "Next after aug-cc-pvqz (top rung) should be None"

    assert get_previous_basis("jun-cc-pvqz") == "aug-cc-pvtz", \
        "Previous before jun-cc-pvqz should be aug-cc-pvtz"
    assert get_previous_basis("aug-cc-pvdz") == "jun-cc-pvdz", \
        "Previous before aug-cc-pvdz should be jun-cc-pvdz"
    assert get_previous_basis("jun-cc-pvdz") is None, \
...
def test_full_basis_ladder_content():
    ...
    assert BASIS_LADDER == expected_ladder, \
