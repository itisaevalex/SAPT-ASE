...
def create_dummy_result(
    ...
    simplified_energies = {
        INTERNAL_KEY_MAP.get(k, k.lower().replace(" ", "_")): v
...
mol_b = Molecule(
    symbols=dummy_symbols, coordinates=dummy_coords + np.array([2.0, 0.0, 0.0])
)
...
def test_run_adaptive_converges():
    ...
    start_task = SaptTask(id="converge_test", monomer_a=mol_a, monomer_b=mol_b, basis_set=BASIS_LADDER[0])
...
def test_run_adaptive_reaches_max_rung():
    ...
    start_task = SaptTask(id="max_rung_test", monomer_a=mol_a, monomer_b=mol_b, basis_set=BASIS_LADDER[0])
...
def test_run_adaptive_fails_midway():
    ...
    mock_backend = MockAdaptiveBackend(
        results_by_basis=mock_energies,
        fail_on_basis=basis_to_fail
    )
    adaptive_options = {"target_accuracy": target_accuracy, "max_rung": 3}

    start_task = SaptTask(id="fail_test", monomer_a=mol_a, monomer_b=mol_b, basis_set=BASIS_LADDER[0])
...
def test_check_convergence(
    ...
    workflow = AdaptiveWorkflow(tasks=[dummy_task], adaptive_options={"target_accuracy": target_accuracy})
