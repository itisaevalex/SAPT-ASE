...
from saptase.core.models import SaptTask, Molecule, TaskStatus
...
def test_psi4_scf_recovery_success_on_second_attempt(mock_psi4, psi4_backend, sample_task):
    ...
    mock_psi4.energy.side_effect = [
        PSI4_CONVERGENCE_ERROR("SCF failed on attempt 1", 99, MagicMock(), 1e-5, 1e-5), # Fails first call
    ...
    }.get(key, 0.0) # Default to 0.0 if key not found
...
def test_psi4_scf_failure_all_attempts(mock_psi4, psi4_backend, sample_task):
    ...
    assert "Last error: Could not converge SCF failed on attempt 4 in 99 iterations." in result.error_message
